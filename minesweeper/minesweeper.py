...
class MinesweeperAI():
    ...
    def process_new_knoledge(self, moves, count):
        new_moves = set()
        for move in moves:
            if move in self.mines:
                count -= 1
            if (not any([move[0] < 0, move[0] >= self.height, move[1] < 0, move[1] >= self.width]) and
                move not in self.safes and move not in self.mines):
                new_moves.add(move)
        return new_moves, count
    ...
    def add_knowledge(self, cell, count):
        ...
        for sentence_a in self.knowledge:
            for sentence_b in self.knowledge:
                if not(sentence_a == sentence_b) and sentence_a.cells.issubset(sentence_b.cells):
        ...
        print(self.safes)
        print('================SAFES_NO_MADE=================')
        print([x for x in self.safes if not x in self.moves_made])
